...
async def get_seller_revenue_analytics(
    ...
    total_revenue = db.query(func.coalesce(func.sum(DBOrderItem.unit_price * DBOrderItem.quantity), 0)).join(
    ...
    monthly_revenue = monthly_revenue_q.all()
    revenue_trends = [
...
async def get_seller_order_analytics(
    ...
    total_orders_q = db.query(func.count(func.distinct(DBOrderItem.order_id))).join(
        DBProduct, DBOrderItem.product_id == DBProduct.id
    ).filter(
        DBProduct.seller_id == seller_id
    )
    ...
    status_counts_q = db.query(DBOrder.status, func.count(func.distinct(DBOrder.id))).join(
    ...
    avg_per_orders = [row[0] for row in avg_order_value_q.all()]
    ...
    monthly_orders = monthly_orders_q.all()
    order_trends = [
